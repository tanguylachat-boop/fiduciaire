"""CLI : export écritures validées → XML Crésus.

Usage :
  python worker/scripts/cresus_export.py \\
    --client-id pilote-jura-01 \\
    --output exports/pilote-jura-01-2026-q2.cresus.xml \\
    --date-from 2026-04-01 --date-to 2026-06-30

  # Dry-run preview (pas de fichier, pas de mark)
  python worker/scripts/cresus_export.py \\
...
from fiduciaire_worker.cresus_export import export_to_cresus_xml  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export entries validées → XML Crésus.",
    )
    ...
    parser.add_argument("--no-mark", action="store_true",
                        help="N'écrit pas cresus_exported_at après export.")
